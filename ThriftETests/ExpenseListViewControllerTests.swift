...
class ExpenseListViewControllerTests: BaseUnitTest {
    ...
    func testExpenseCellOrderingAndMetadataAndCorrect() {
        ...
        let expense1 = coreDataManager.insertExpense(name: "test1", amount: 10.00, category: "Misc", date: date1!)
        let expense2 = coreDataManager.insertExpense(name: "test2", amount: 20.00, category: "Misc", date: date2!)
        let expense3 = coreDataManager.insertExpense(name: "test3", amount: 30.00, category: "Misc", date: date3!)
        ...
        XCTAssertTrue(cell1?.detailLabel.text == "$10.0 \(dateFormatter.string(from: date1!))")
        XCTAssertTrue(cell2?.nameLabel.text == "test3")
        XCTAssertTrue(cell2?.detailLabel.text == "$30.0 \(dateFormatter.string(from: date3!))")
